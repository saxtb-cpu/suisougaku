...
applyAll(G,{ens:3,morale:3});
// グループ内の競争で個人技向上
G.members.filter(m=>Math.random()<0.4).forEach(m=>{
  m.skill=cap(m.skill+rnd(1,3));
  m.expression=cap(m.expression+rnd(1,2));
});
showEventPopup('🎶','アンサンブルコンテスト準備開始',
  `${groupCount}つの小編成グループを編成した。パート練習とは違う緊張感が部員たちを成長させる。\nグループ内の個人技磨きが始まった。`,'var(--blue)');
addLog(`アンコン${groupCount}グループ編成。個人技練習開始。`,'イベント');
}
