...
const PARTS=['フルート','クラリネット','サックス','トランペット','トロンボーン','ホルン','ユーフォニアム','チューバ','打楽器','オーボエ'];
...
const PRAC_SONG_FX={
  ...
  festival:   {pitch:0, rhythm:0, tempo:0, expression:0, balance:0}, // コンクール曲には影響しない
};
const PART_W={フルート:12,クラリネット:18,サックス:12,トランペット:14,トロンボーン:10,ホルン:8,ユーフォニアム:6,チューバ:4,打楽器:13,オーボエ:7};
const PART_COL={フルート:'#2a8aaa',クラリネット:'#3a7a50',サックス:'#a07020',トランペット:'#b04020',トロンボーン:'#7040a0',ホルン:'#1a6090',ユーフォニアム:'#8060a0',チューバ:'#804040',打楽器:'#606060',オーボエ:'#406040'};
const PART_BG={フルート:'#e8f5fa',クラリネット:'#edf7f1',サックス:'#fdf6e6',トランペット:'#fdf0ec',トロンボーン:'#f5eeff',ホルン:'#e8f3fa',ユーフォニアム:'#f0ecff',チューバ:'#fdf0f0',打楽器:'#f5f5f5',オーボエ:'#edf5ee'};
...
{id:'festival', icon:'🎉',name:'文化祭曲練習', desc:'文化祭用のポップな選曲を仕上げる。8〜9月のみ有効。コンクール曲とは別に磨く必要がある。', fx:{skill:1,ens:3,song:0, morale:6, stamina:1},cost:0,tag:'festival',festivalOnly:true},
];
...
PARTS.forEach(p=>{
  ...
  if(REQUIRED_PARTS.includes(p)){
    alloc[p]=Math.max(1,Math.round(raw));
  } else {
    // 少人数では希少パートは0になることがある
    const minThresh=count<22?0.18:count<30?0.12:0.06;
    if(raw<minThresh*count){
      alloc[p]=Math.random()<(raw/(minThresh*count))?1:0;
    } else {
      alloc[p]=Math.max(0,Math.round(raw));
    }
  }
});
...
while(diff<0){
  const shrinkable=PARTS.filter(p=>alloc[p]>1&&!REQUIRED_PARTS.includes(p));
  const target=shrinkable.length>0?pick(shrinkable):PARTS.find(p=>alloc[p]>1);
  if(target){alloc[target]--;diff++;}else break;
}
PARTS.forEach(part=>{
...
if(isExamWeek()){
  document.getElementById('week-grid').innerHTML=`
    <div style="grid-column:1/-1;background:var(--amber2);border:1.5px solid var(--amber);border-radius:10px;padding:18px;text-align:center">
      <div style="font-size:28px;margin-bottom:8px">📝</div>
      <div style="font-family:var(--serif);font-size:15px;font-weight:700;color:var(--amber);margin-bottom:6px">期末試験週</div>
      <div style="font-size:12px;color:var(--ink2)">今週は練習短縮・禁止です。<br>部員たちは勉強に集中しています。<br>スタミナが回復しますが、技術は伸びません。</div>
    </div>`;
  document.getElementById('prac-preview').style.display='none';
  document.getElementById('sel-prac-name').textContent='試験週（選択不可）';
  return;
}
const isSeason=G.month>=7&&G.month<=10;
const isFestivalSeason=G.month===8||G.month===9;
document.getElementById('week-grid').innerHTML=PRACTICES
.filter(p=>!p.festivalOnly||isFestivalSeason)
.map((p,i)=>{
...
if((p.tag||p.id)==='festival') G._festivalPrac=(G._festivalPrac||0)+1;
...
const pracOpenings={
  ...
  festival:[
    `コンクール曲とは違う、ポップな曲を練習する。なんだか新鮮で、部員たちの表情が柔らかくなった。`,
    `「文化祭らしい曲にしよう」——${cap_?cap_.name+'が':'部長が'}選曲会議を仕切った。いつもと違う雰囲気で、練習室が少し明るくなった。`,
    `文化祭用の曲に取り組んだ今週。コンクール曲の緊張感と違う楽しさがあった。`,
    `一般のお客さんに届く演奏を作ろう。${mem?mem.name+'は':'部員たちは'}そう意識しながら、いつもと違う表現を探していた。`,
  ],
};
...
const pracEndings={
  ...
  festival:[
    `文化祭まであとわずか。コンクール曲とは違う緊張感が、じわじわと近づいてきた。`,
    `一般のお客さんに届く演奏を目指して、今週も丁寧に仕上げた。本番が楽しみだ。`,
    `楽しい曲は、楽しく演奏する。それが意外と難しいと気づいた一週間だった。`,
  ],
};
const commonEndings=[
  `来週もまた、全国への道は続く。`,
...
const newCount=base+bonus+festBonus;
const cfg=DIFF[G.diff];
// 新入部員は希少パートを選びにくい
...
const festNote=festBonus>0?`文化祭演奏の好評で+${festBonus}名増！`:'';
const recruitMsg=hasNatGold?`全国・東日本大会金賞の効果で${newCount}名（+${bonus}名増）の新入部員が来た！${festNote}`
:hasPrefGold?`支部大会金賞の知名度で${newCount}名（+${bonus}名増）が入部した。${festNote}`
:hasGold?`コンクール金賞の知名度で${newCount}名（+${bonus}名増）が入部した。${festNote}`
:festBonus>0?`文化祭の演奏が好評で${newCount}名（+${festBonus}名増）が入部しました！`
...
function openSpringConcertModal(upperCount){
  ...
  if(!body){
    // モーダルがない場合はconfirmで代替
    const hold=confirm(`2・3年生が${upperCount}人です。春季定期演奏会を開催しますか？\n\n開催する → OK\n見送る → キャンセル`);
    decideSpringConcert(hold);
    return;
  }
  body.innerHTML=`
    <div style="font-size:12px;color:var(--ink2);margin-bottom:12px">
      現在の2・3年生：<strong style="color:var(--ink)">${upperCount}人</strong><br>
      通常30人以上で開催していますが、どうしますか？
    </div>
    <div style="background:var(--bg);border-radius:8px;padding:10px 12px;margin-bottom:12px;font-size:11px;color:var(--ink3)">
      少人数でも心のこもった演奏会にすることは可能です。<br>
      ただし会場費・準備費用はかかります。
    </div>
    <div style="display:flex;flex-direction:column;gap:8px">
      <button class="btn btn-gold" style="padding:12px" onclick="decideSpringConcert(true)">
        🎹 開催する（${upperCount}人で頑張る！）
      </button>
      <button class="btn" style="padding:12px" onclick="decideSpringConcert(false)">
        📋 今年は見送る
      </button>
    </div>`;
  document.getElementById('ov-spring-concert').classList.add('show');
}
function decideSpringConcert(hold){
  G._springConcert=hold;
  if(document.getElementById('ov-spring-concert')){
    closeModal('ov-spring-concert');
  }
  const upper=G.members.filter(m=>m.grade>=2).length;
  if(hold){
    notif('春季演奏会','開催を決定しました！5月に向けて準備を始めましょう🎺');
    addLog(`春季演奏会：開催決定（2・3年生${upper}人）`,'イベント');
  } else {
    notif('春季演奏会','今年は見送りにしました。');
    addLog(`春季演奏会：見送り決定（2・3年生${upper}人）`,'イベント');
  }
}
...
// 1年目は春季演奏会なし
if(G.year === 1){
  addLog('1年目のため春季演奏会はなし。','システム');
  return;
}
// 開催決定していれば演奏会を実施
if(G._springConcert === true){
  runSpringConcert();
} else if(G._springConcert === false){
  showEventPopup('📋','春季演奏会：今年は見送り',
    '今年の春季定期演奏会は部員数の関係で見送りになった。2・3年生は次のステージに向けて気持ちを切り替えた。','var(--ink3)');
  addLog('春季演奏会：今年は見送り。','イベント');
} else {
  // 判断が行われなかった場合（念のため）
  addLog('春季演奏会：開催判断なし。','システム');
}
}


function runExamWeek(){
  recalc();
  const before={skill:G.skill,ensemble:G.ensemble,song:G.song,morale:G.morale,funds:G.funds,
    kadaiParams:G.kadaiParams?Object.assign({},G.kadaiParams):newSongParams(0),
    jiyuParams:G.jiyuParams?Object.assign({},G.jiyuParams):newSongParams(0)};
  ...
  // スタミナ回復（休息）
  G.members.forEach(m=>{
    m.stamina = cap(m.stamina + rnd(3,7));
    // 士気は微妙：勉強のストレスで少し下がるが個人差あり
    const studyStress = rnd(-3,2);
    m.morale = cap(m.morale + studyStress);
    // 技術は維持（上がらない・少し落ちる可能性）
    if(m.stamina < 40 && Math.random() < 0.15){
      m.skill = cap(m.skill - 1);
    }
  });

  // 曲パラメーターも伸びない（練習なし）
  // ※kadaiParams/jiyuParamsはそのまま変化なし
  ...
  if(G.week>=WEEKS_PER_MONTH){
    G.week=1;
    G._pendingMonthAdvance=true;
  } else {
    G.week++;
  }
  addLog('期末試験週：練習短縮。スタミナ回復。','試験');
  showExamWeekResult(before, after);
}

async function showExamWeekResult(before, after){
  ...
  if(el_title) el_title.textContent='試験週の記録';
  if(el_prac) el_prac.innerHTML='<span class="wr-prac-label" style="background:var(--amber);color:#fff">📝 期末試験週</span>';
  if(el_event) el_event.innerHTML=`
    <div class="wr-event-badge" style="border-color:var(--amber)">
      <div class="wr-event-icon">📝</div>
      <div>
        <div class="wr-event-title">期末試験週</div>
        <div class="wr-event-body">今週は練習短縮・禁止。部員たちは勉強に集中した。体は少し休めたが、楽器から離れる寂しさもある。</div>
      </div>
    </div>`;
  ...
  // スタミナ変化を別途表示
  const avgStaminaBefore=Math.round(G.members.reduce((a,m)=>a+m.stamina,0)/G.members.length);
  if(el_rows) el_rows.innerHTML+=`
    <div style="background:var(--bg3);border-radius:8px;padding:8px 11px;margin-top:8px;font-size:11px;color:var(--ink2)">
  ...
  if(sw){
    ...
    const mem=G.members.length>0?pick(G.members):null;
    const stories=[
      `試験期間中、練習室は静かだった。${mem?mem.name+'は':'部員たちは'}教科書を広げながらも、ふと楽器のことを思った。「早く弾きたい」——その気持ちが、来週の練習への原動力になった。`,
      `「勉強しなきゃ」とわかっていても、${mem?mem.name+'の':''} 頭の中ではずっと課題曲が流れていた。試験が終わったら、あのフレーズをもう一度ちゃんと吹いてみよう。`,
      `一週間の試験期間。練習できないもどかしさはあったが、体を休める時間にもなった。コンクールまであとわずか。試験明けの練習で、また気持ちを引き締めよう。`,
      `${G.advisor}先生から「試験週は勉強に集中しなさい」と言われていた。${mem?mem.name+'は':''} 楽器ケースをじっと見つめながら、来週の練習を心待ちにしていた。`,
    ];
...
// チケット収入：2・3年生の人数・スキル・士気で変動
const avgSkill  = Math.round(upperMembers.reduce((a,m)=>a+m.skill,0)/upper);
const avgMorale = Math.round(upperMembers.reduce((a,m)=>a+m.morale,0)/upper);
const baseAudience = upper>=30 ? 120 : upper>=25 ? 80 : 50;
const audience = Math.max(20, baseAudience
...
G.funds += net;
// 演奏の出来で士気・アンサンブルが変わる
const quality = avgSkill >= 65 && avgMorale >= 60;
const moraleGain = quality ? rnd(6,10) : rnd(2,6);
const ensGain    = quality ? rnd(3,6)  : rnd(1,3);
// 2・3年生に効果（主役なので）
upperMembers.forEach(m=>{
  m.morale     = cap(m.morale + moraleGain + rnd(-2,2));
  m.expression = cap(m.expression + rnd(1,3));
});
// 1年生は演奏を観て刺激を受ける
G.members.filter(m=>m.grade===1).forEach(m=>{
  m.morale = cap(m.morale + rnd(3,6));
});
const sizeLabel = upper>=30 ? `${audience}人が来場し盛況でした` : upper>=25 ? `${audience}人が来場。こじんまりとした演奏会でした` : `${audience}人の前での演奏。小規模ながら心のこもった舞台でした`;
const qualityMsg = quality ? '2・3年生の実力が存分に発揮された、充実した演奏会になった。' : '課題は残ったが、先輩たちの想いのこもった演奏が客席に届いた。';
showEventPopup('🎹','春季定期演奏会',
  `${sizeLabel}。\n${qualityMsg}\n収支：${net>=0?'+':''}${net.toLocaleString()}円（入場${income.toLocaleString()}円 / 経費${cost.toLocaleString()}円）`,
  quality?'var(--gold)':'var(--teal)');
addLog(`春季定期演奏会（2・3年生${upper}人出演）観客${audience}人 収支${net>=0?'+':''}${net.toLocaleString()}円`,'イベント');
}
...
G.members.filter(m=>Math.random()<0.3).forEach(m=>{
  m.skill=cap(m.skill+rnd(1,2));
  m.expression=cap(m.expression+rnd(1,2));
});
...
if(preSuccess){
  ...
  showEventPopup('🎟','定期演奏会：前売り好調',
    `前売りチケットが${preTicket.toLocaleString()}円分売れた！来てくれる人がいると思うと、部員のやる気も上がる。`,'var(--gold)');
  addLog(`定期演奏会前売り収入：+${preTicket.toLocaleString()}円`,'資金');
} else {
  applyAll(G,{morale:2});
...
const income=rnd(15000,35000);
const cost=rnd(10000,20000);
const net=income-cost;
G.funds+=net;
const success=G.skill>45&&G.morale>50;
if(success){
  applyAll(G,{morale:8,ens:3,skill:1});
  showEventPopup('🎄','年末発表会 大成功',
    `1年間の成長を披露した年末発表会。3年生への感謝を込めた演奏に、観客から温かい拍手が送られた。\n収支：${net>=0?'+':''}${net.toLocaleString()}円`,'var(--gold)');
  addLog(`年末発表会成功。収支${net>=0?'+':''}${net.toLocaleString()}円`,'資金');
} else {
  applyAll(G,{morale:4,ens:2});
  showEventPopup('🎄','年末発表会',
    `年末の発表会を行った。課題は残ったが、1年間の積み重ねを感じる演奏になった。\n収支：${net>=0?'+':''}${net.toLocaleString()}円`,'var(--teal)');
  addLog(`年末発表会。収支${net>=0?'+':''}${net.toLocaleString()}円`,'資金');
}
}
function monthEvent1(){
// ── 春季演奏会の開催判断（2年目以降） ──
if(G.year >= 2 && G._springConcert === null){
  const upperMembers = G.members.filter(m=>m.grade>=2);
  const upper = upperMembers.length;
  if(upper >= 30){
    G._springConcert = true;
    notif('春季演奏会','2・3年生が'+upper+'人！春季定期演奏会の開催を決定しました🎺');
    addLog(`春季演奏会：開催決定（2・3年生${upper}人）`,'イベント');
  } else if(upper >= 20){
    // プレイヤーに選択させる
    setTimeout(()=>openSpringConcertModal(upper), 800);
  } else {
    G._springConcert = false;
    notif('春季演奏会','2・3年生が'+upper+'人のため、今年の春季演奏会は見送りになりました。');
    addLog(`春季演奏会：見送り（2・3年生${upper}人）`,'イベント');
  }
}
// アンコン本選：部のアンサンブル力・個人技で結果が変わる
const baseScore=Math.round(G.ensemble*0.5+G.skill*0.3+G.morale*0.2);
const variance=rnd(-8,12);
const score=Math.min(99,Math.max(30,baseScore+variance));
const result=score>=82?'金賞':score>=68?'銀賞':score>=52?'銅賞':'入賞なし';
const col=result==='金賞'?'var(--gold)':result==='銀賞'?'#888':result==='銅賞'?'var(--amber)':'var(--ink3)';
const moraleGain=result==='金賞'?12:result==='銀賞'?5:result==='銅賞'?1:-4;
const ensGain=result==='金賞'?6:result==='銀賞'?3:2;
applyAll(G,{ens:ensGain,morale:moraleGain,skill:2});
// 参加部員の個人技向上
G.members.filter(m=>Math.random()<0.3).forEach(m=>{
  m.skill=cap(m.skill+rnd(1,3));
  m.expression=cap(m.expression+rnd(1,2));
...
const msgs={
  '金賞':'小編成チームが金賞に輝いた！個人技の高さが審査員に評価された。部全体の士気が一気に上がった。',
  '銀賞':'銀賞という結果に終わったが、個々の成長は確かだ。来年の全体演奏に活かしていこう。',
  '銅賞':'銅賞を獲得した。悔しさもあるが、小編成ならではの経験が部員を成長させた。',
  '入賞なし':'入賞は逃したが、小編成での演奏経験は大きな財産になった。それぞれの課題が明確になった。',
};
...
if(grad3.length>0){
  // 送別演奏会 + 引き継ぎ
  const underMorale=G.members.filter(m=>m.grade<3);
  underMorale.forEach(m=>m.morale=cap(m.morale+rnd(3,8)));
  // 3年生から後輩への技術継承（少しスキルが伝わる）
  const avgGrad3Skill=Math.round(grad3.reduce((a,m)=>a+m.skill,0)/grad3.length);
  G.members.filter(m=>m.grade<3).forEach(m=>{
    if(avgGrad3Skill>m.skill&&Math.random()<0.3){
      m.skill=cap(m.skill+rnd(1,2));
    }
  });
  showEventPopup('🎓','卒業式・送別演奏',
    `卒業式で${grad3.length}名の先輩を演奏で送り出した。\n先輩から後輩へ、技術と想いが受け継がれた。来年度への誓いを新たにした。`,'var(--gold)');
  addLog(`卒業式演奏。${grad3.length}名の先輩から技術継承。`,'イベント');
  // 楽器・備品の引き継ぎ確認
  if(Math.random()<0.4){
    const cost=rnd(5000,20000);
    if(G.funds>=cost){
      G.funds-=cost;
      applyAll(G,{skill:1,stamina:2});
      notif('備品整備',`新年度に向け備品を整備しました（-${cost.toLocaleString()}円）`);
      addLog(`備品整備（-${cost.toLocaleString()}円）`,'資金');
    }
  }
} else {
  showEventPopup('🌸','新年度準備',
    '卒業生はいなかったが、新年度に向けて気持ちを新たにした。来年度の目標を部員全員で共有した。','var(--teal)');
  applyAll(G,{morale:5});
  addLog('新年度準備完了。目標を共有。','イベント');
}
}
...
function openSongDetail(){
  if(!G.kaDaiKyoku){
    notif('曲未決定','6月に課題曲・自由曲が決まると確認できます');
    return;
  }
  const hasKadai = !!G.kaDaiKyoku.selectedKadai;
  const hasJiyu  = !!G.kaDaiKyoku.selectedJiyu;
  if(!hasKadai && !hasJiyu){
    notif('選曲未完了','課題曲と自由曲を選んでから確認できます');
    return;
  }

  const kadaiAvg = songAvg(G.kadaiParams||newSongParams());
  const jiyuAvg  = songAvg(G.jiyuParams||newSongParams());
  const totalAvg = hasKadai&&hasJiyu ? Math.round((kadaiAvg+jiyuAvg)/2)
                 : hasKadai ? kadaiAvg : jiyuAvg;

  document.getElementById('sd-subtitle').textContent =
    `${G.year}年度 ／ 総合平均精度：${totalAvg} / 100`;

  // パラメーターごとのバー + 両曲比較
  const paramRows = SONG_PARAMS.map(sp=>{
    const kv = G.kadaiParams?.[sp.key]??0;
    const jv = G.jiyuParams?.[sp.key]??0;
    const avg = hasKadai&&hasJiyu ? Math.round((kv+jv)/2) : hasKadai ? kv : jv;
    const barW = avg;
    const grade = avg>=80?'◎':avg>=60?'○':avg>=40?'△':'✗';
    const gradeCol = avg>=80?'var(--green)':avg>=60?'var(--teal)':avg>=40?'var(--amber)':'var(--red)';
    // 両曲の個別バー
    const kadaiBar = hasKadai ? `
      <div style="display:flex;align-items:center;gap:4px;margin-bottom:2px">
        <span style="font-size:9px;color:var(--ink3);width:36px">課題曲</span>
        <div style="flex:1;height:5px;background:var(--bg4);border-radius:3px;overflow:hidden">
          <div style="width:${kv}%;height:100%;background:#2a5fa8;border-radius:3px"></div>
        </div>
        <span style="font-size:10px;color:#2a5fa8;font-weight:700;width:24px;text-align:right">${kv}</span>
      </div>` : '';
    const jiyuBar = hasJiyu ? `
      <div style="display:flex;align-items:center;gap:4px">
        <span style="font-size:9px;color:var(--ink3);width:36px">自由曲</span>
        <div style="flex:1;height:5px;background:var(--bg4);border-radius:3px;overflow:hidden">
          <div style="width:${jv}%;height:100%;background:#1a8070;border-radius:3px"></div>
        </div>
        <span style="font-size:10px;color:#1a8070;font-weight:700;width:24px;text-align:right">${jv}</span>
      </div>` : '';

    return `
    <div style="background:var(--bg);border-radius:8px;padding:10px 12px;margin-bottom:8px">
      <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px">
        <span style="font-size:15px">${sp.icon}</span>
        <span style="font-size:12px;font-weight:700;flex:1">${sp.label}</span>
        <span style="font-size:13px;font-weight:800;color:${gradeCol}">${grade}</span>
        <span style="font-size:12px;color:${sp.col};font-weight:700">${avg}</span>
      </div>
      ${kadaiBar}${jiyuBar}
    </div>`;
  }).join('');
  ...
  const radarSection = `
    <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:12px">
      ${hasKadai?`<div style="flex:1;min-width:160px">
        <div style="font-size:10px;color:var(--ink3);text-align:center;margin-bottom:4px">
          📘 課題曲「${G.kaDaiKyoku.selectedKadai.title}」<br>
          <strong style="color:#2a5fa8">平均 ${kadaiAvg}</strong>
        </div>
        <svg id="sd-radar-kadai" viewBox="0 0 200 200" style="width:100%"></svg>
      </div>`:''}
      ${hasJiyu?`<div style="flex:1;min-width:160px">
        <div style="font-size:10px;color:var(--ink3);text-align:center;margin-bottom:4px">
          📗 自由曲「${G.kaDaiKyoku.selectedJiyu.title}」<br>
          <strong style="color:#1a8070">平均 ${jiyuAvg}</strong>
        </div>
        <svg id="sd-radar-jiyu" viewBox="0 0 200 200" style="width:100%"></svg>
      </div>`:''}
    </div>`;

  // 弱点アドバイス
  const weakParams = SONG_PARAMS.filter(sp=>{
    const v = hasKadai&&hasJiyu
      ? Math.round(((G.kadaiParams?.[sp.key]??0)+(G.jiyuParams?.[sp.key]??0))/2)
      : hasKadai ? (G.kadaiParams?.[sp.key]??0) : (G.jiyuParams?.[sp.key]??0);
    return v < 50;
  });
  const adviceMap = {
    pitch:      '🎵 基礎練習・パート練習で音程を鍛えましょう',
    rhythm:     '🥁 合奏練習・録音振り返りでリズムを整えましょう',
    tempo:      '⏱ 個人特訓・パート練習でテンポを安定させましょう',
    expression: '🎭 メンタル強化・ライバル研究で表現力を磨きましょう',
    balance:    '🎚 合奏練習・パート練習（顧問巡回）でバランスを整えましょう',
  };
  const adviceHtml = weakParams.length > 0
    ? `<div style="background:var(--amber2);border:1px solid var(--amber);border-radius:8px;padding:10px 12px;margin-bottom:10px">
        <div style="font-size:10px;font-weight:700;color:var(--amber);margin-bottom:6px">⚠ 改善が必要な項目</div>
        ${weakParams.map(sp=>`<div style="font-size:11px;color:var(--ink2);margin-bottom:3px">${adviceMap[sp.key]||sp.label}</div>`).join('')}
      </div>`
    : `<div style="background:var(--green2);border:1px solid var(--green);border-radius:8px;padding:8px 12px;margin-bottom:10px;font-size:11px;color:var(--green)">
        ✅ 全パラメーターが50以上です！バランスよく仕上がっています。
      </div>`;

  document.getElementById('sd-body').innerHTML = radarSection + adviceHtml + paramRows;
  document.getElementById('ov-song-detail').classList.add('show');

  // レーダー描画（モーダル表示後）
  setTimeout(()=>{
    if(hasKadai) drawRadarChart('sd-radar-kadai', G.kadaiParams, '#2a5fa8');
    if(hasJiyu)  drawRadarChart('sd-radar-jiyu',  G.jiyuParams,  '#1a8070');
  }, 50);
}
